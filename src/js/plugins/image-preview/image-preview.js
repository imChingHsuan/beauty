...
//讀取檔案
function readFileHandler(e, input, params) {
  ...
  reader.onload = (event) => {
    ...
    if (params.sizeLimit && !checkSize(file, params.sizeLimit)) {
      input.value = '';
      parentsEl.classList.add('over-limit');
      if (params.on.overLimit && typeof params.on.overLimit === 'function')
        params.on.overLimit(input);
      return;
    } else {
      parentsEl.classList.remove('over-limit');
      parentsEl.classList.add('uploaded');
      convertImg(params, dataUrl).then((res) => {
        createDOM(input, params, file, res);
      });
    }
  };
  ...
  init() {
    ...
    inputs.forEach((input, index) => {
      ...
      const params = {
        ...
        on: {
          ...
          overLimit: input.dataset.overLimit
            ? function (el) {
                eval(input.dataset.overLimit + '(el);');
              }
      ...
      input.addEventListener('change', function (e) {
        readFileHandler(e, input, params);
      });
    });
  }
}
